...
def compute_score(predicts: List[str], ground_truths: List[str], images: List[str], format_weight: float = 0.1, file_path: str = "", historical_scores: Optional[List[float]] = None) -> List[Dict[str, float]]:
    results = []
    ...
    for i in range(len(predicts)):
        ...
        # New reward function: 1 - |item["score"] - (1 - historical_accuracy)| + base_score
        flip_success = 0
        ...
        if item['question']:
            ...
            adjusted_score = 1 - alignment_penalty + min(item["score"], 1-item["score"]) #1 - alignment_penalty +
