...
from ..workers.reward import BatchFunctionRewardManager, SequentialFunctionRewardManager, BatchFunctionRewardManagerRL
...
class Runner:
    ...
    def run(self, config: PPOConfig):
        ...
        # }
        FSDPWorkerRemote = ray.remote(num_gpus=1)(FSDPWorker)
        role_worker_mapping = {
            Role.ActorRollout: FSDPWorkerRemote,
            Role.Critic: FSDPWorkerRemote,
            Role.RefPolicy: FSDPWorkerRemote,
        }
        ...
        elif config.worker.reward.reward_type == "batchrl":
            RewardManager = BatchFunctionRewardManagerRL
        else:
...
def main():
    ...
    os.environ['VLLM_LOGGING_LEVEL'] = 'WARNING'  # 从WARN改为WARNING
